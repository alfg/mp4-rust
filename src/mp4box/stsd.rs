...
use crate::mp4box::*;
use crate::mp4box::{avc1::Avc1Box, hevc::HevcBox, mp4a::Mp4aBox, tx3g::Tx3gBox};
...
impl StsdBox {
    ...
    pub fn get_size(&self) -> u64 {
        ...
        if let Some(ref avc1) = self.avc1 {
            size += avc1.box_size();
        } else if let Some(ref hevc) = self.hevc {
            size += hevc.box_size();
    ...
    fn read_box(reader: &mut R, size: u64) -> Result<Self> {
        ...
        match name {
            ...
            BoxType::Hev1Box | BoxType::Hvc1Box => {
                hevc = Some(HevcBox::read_box(reader, s)?);
            }
            BoxType::Vp09Box => {
